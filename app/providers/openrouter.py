...
class OpenRouterProvider(BaseProvider):
    ...
    def __init__(self, api_key, timeout, referer, title):
        self.api_key, self.timeout, self.referer, self.title = api_key, timeout, referer, title

    def _headers(self):
        return {'Authorization': f'Bearer {self.api_key}', 'Content-Type': 'application/json', 'HTTP-Referer': self.referer, 'X-Title': self.title}
    ...
    async def health(self) -> ProviderHealth:
        ...
        try:
            async with httpx.AsyncClient(timeout=min(self.timeout, 8.0)) as client:
                r = await client.get('https://openrouter.ai/api/v1/models', headers=self._headers())
